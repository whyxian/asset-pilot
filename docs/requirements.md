...
以**持仓为主、交易为辅**，提供持仓管理、盈亏计算、简单年化回报率分析、净值走势追踪、现金账户管理的核心功能。
...
**功能点：**
- ✅ 查看交易记录列表（按日期倒序，分页）
- ✅ 新增交易记录（日期、品种、方向、数量、单价、金额、备注；buy/sell 自动联动现金流水）
- ✅ 编辑/删除已有交易记录（联动同步/回退现金流水）
- ✅ 支持按品种筛选
- ✅ 分页（page/page_size，默认 20 条）
- ~~手续费字段~~——未来优化项，当前不实现
...
**描述：** 独立于交易系统的资金流水管理，记录所有资金进出（入金/出金/买卖扣款入账），是现金余额的事实源。

**页面布局：** 左右分栏——左侧 sticky 余额卡片，右侧流水列表。

**功能点：**
- ✅ 各币种现金余额展示，总额按显示币种（默认 CNY）用实时汇率换算（对齐概览页模式，页脚展示汇率日期，兜底时橙色警告）
- ✅ 入金（`POST /api/v1/cash/deposit`）：记录正流水
- ✅ 出金（`POST /api/v1/cash/withdraw`）：记录负流水，**校验同币种余额充足**
- ✅ 流水列表（时间倒序，分页），支持删除单笔流水
- ✅ **买卖自动联动**：新建 buy 交易 → 自动生成扣款流水（校验余额）；新建 sell 交易 → 自动生成入账流水；更新/删除交易 → 同步更新/回退关联流水
- ✅ **建仓联动**：建仓时勾选"参与现金账户" → 从现金余额扣款（校验余额）；不勾选 → 自动先入金等额（"建仓 XX 自动入金（历史本金）"）再扣款，代表历史本金注入。**现金追踪本身永远开，勾选只决定建仓资金从哪来**
- ✅ 余额数据源：`GET /api/v1/cash/balances?currency=`（汇率换算 + 单飞复用 fetch_rates）
